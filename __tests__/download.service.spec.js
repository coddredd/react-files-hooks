...
describe('DownloadService methods', () => {
    ...
    describe('download method', () => {
        ...
        it('should download photo (base64) with window location', () => {
            ...
            downloadService._isSafari = jest.fn().mockImplementationOnce(() => true);
        ...
        it('should download photo (big base64) with DOM element as big base64', (done) => {
            ...
            downloadService._isBigBase64 = jest.fn().mockImplementationOnce(() => true);
            downloadService._dataUrlToBlob = jest.fn().mockImplementationOnce(() => 'blob');

            downloadService.download(base64, strFileName, strMimeType);
            expect(downloadService._dataUrlToBlob).toHaveBeenCalledWith(base64, expect.any(Function));
            expect(setAttributeMock).toHaveBeenCalledWith('download', strFileName);
            expect(document.body.appendChild).toHaveBeenCalledWith({
                download: downloadMock,
                setAttribute: setAttributeMock,
